function feature = import_idt_hog3d (file, tra_len)
...
%       feature:  imported idt features
    ...
    fid = fopen(file,'rb');
    feat = fread(fid,[10+4*tra_len+96*4+108,inf],'float');
	feature = struct('info',[],'tra',[],'tra_shape',[],'hog',[],'hof',[],'mbhx',[],'mbhy',[],'hog_depth',[]);
	if ~isempty(feat)
		...
		feature.mbhy = feat(ind+301:ind+396,:);
        feature.hog_depth = feat(ind+397:end,:);
	end
    fclose(fid);
end
