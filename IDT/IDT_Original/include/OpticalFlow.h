...
namespace my
{
    ...
    void FarnebackPolyExpPyr(const Mat& img, std::vector<Mat>& poly_exp_pyr, std::vector<float>& fscales, 
    ...
    void writeImages(std::vector<uchar>& images, string name_temp, int frameNum)
    {
            char tmp[256];
            sprintf(tmp, "_%05d.png", frameNum);
            FILE* fp;
            fp = fopen((name_temp + tmp).c_str(), "wb");
            fwrite( images.data(), 1, images.size(), fp);
            fclose(fp);
    }

    /* 
    visualize optical flow pyramid both in x and y dimension
    the filename formats as: flow_x(y)_scale_frame, e.g. flow_x_scale_1_0012
    */
    void VisualOpticalFlow(std::vector<Mat>& flow_pyr, int bound, int frameNum, int scaleNum)
    {
        std::vector<string> flow_name;
        std::stringstream file_name_x, file_name_y;
        string path = "/home/idt_data/"; // example path for output
        flow_name.clear();
        flow_name.push_back("flow_x_scale_");
        flow_name.push_back("flow_y_scale_");

        for(int iScale = 0; iScale < scaleNum; iScale++)
        {
            Mat flow_split[2];
            vector<uchar> str_x, str_y;
            split(flow_pyr[iScale], flow_split);
            encodeFlowMap(flow_split[0], flow_split[1], str_x, str_y, bound, true); 

            file_name_x << path << flow_name.at(0) << iScale;
            file_name_y << path << flow_name.at(1) << iScale;

            writeImages(str_x, file_name_x.str(), frameNum);
            writeImages(str_y, file_name_y.str(), frameNum);
