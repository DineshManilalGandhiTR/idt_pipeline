...
// for HOF descriptor
void HofComp(const Mat& flow_x, const Mat& flow_y, float* desc, DescInfo& descInfo)
{
	BuildDescMat(flow_x, flow_y, desc, descInfo);
}

// for MBH descriptor
void MbhComp(const Mat& flow_x, const Mat& flow_y, float* descX, float* descY, DescInfo& descInfo)
{
	Mat flowXdX, flowXdY, flowYdX, flowYdY;
	Sobel(flow_x, flowXdX, CV_32F, 1, 0, 1);
	Sobel(flow_x, flowXdY, CV_32F, 0, 1, 1);
	Sobel(flow_y, flowYdX, CV_32F, 1, 0, 1);
	Sobel(flow_y, flowYdY, CV_32F, 0, 1, 1);
...
void DrawTrack(const std::vector<Point2f>& point, const int index, Mat& image)
{
